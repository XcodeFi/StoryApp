...
namespace StoryApp.Services.Base
{
    public class LocalRepo<T>:ILocalRepo<T> where T: BaseModel,new ()
    {
        ...
        public LocalRepo(SQLiteAsyncConnection dbAsync, SQLiteConnection db)
        {
            this._db = db;
            this._dbAsync = dbAsync;
            if (!LocalDBExists)
            {
                dbAsync.CreateTableAsync<Story>();
                dbAsync.CreateTableAsync<Word>();
                DbExists();
        ...
        void InitData()
        {
            StoryRepo _story = new StoryRepo();
            WordRepo _word = new WordRepo();
            for (int i = 0; i < 10; i++)
            {
                _story.Insert(new Story { Title = $"title {i}", Content = $"content {i}" });
                _word.Insert(new Word { BaseWord = $"BaseWord {i}", Meaning = $"Meaning {i}" });
            }
        }
    }
}